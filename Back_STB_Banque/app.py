...
# --- IMPORTS POUR L'AUTHENTIFICATION FIREBASE ---
import json
# from auth import signin, signup, logout, parse_firebase_error # Importez vos fonctions d'authentification et l'objet auth si besoin

# --- Configuration Streamlit ---
st.set_page_config(layout="wide", page_title="Analyse PFE - Clients & Fournisseurs")

# Initialize session state variables for authentication and page management
if 'logged_in' not in st.session_state:
    st.session_state.logged_in = False
if 'user_role' not in st.session_state: # Nous n'utiliserons plus des rôles fixes ici, mais un token utilisateur Firebase
    st.session_state.user_role = None
if 'user_info' not in st.session_state: # Pour stocker les infos de l'utilisateur connecté (email, uid, etc.)
    st.session_state.user_info = None
if 'page' not in st.session_state:
    st.session_state.page = "login"

# Initialize chat history for LLM
if 'chat_history' not in st.session_state:
    st.session_state.chat_history = []

# --- Gemini API Configuration (TOP LEVEL) ---
try:
    # Il est préférable de charger la clé API depuis st.secrets
    # Pour ce PFE, si vous n'avez pas de secrets.toml, vous pouvez la laisser directement ici
    # ou utiliser os.environ.get("GOOGLE_API_KEY")
...
# --- Content Functions for Each Section (unchanged) ---
...
# --- New function to download questions ---
...
# --- Login/Signup Page Function (MODIFIED) ---
def login_page():
    st.title("Bienvenue au Tableau de Bord d'Analyse PFE")
    st.info("Veuillez vous connecter ou créer un compte pour accéder au contenu.")

    col1, col2 = st.columns(2)

    with col1: # Section Connexion
        st.subheader("Se connecter")
        login_email = st.text_input("Email (Connexion)", key="login_email")
        login_password = st.text_input("Mot de passe (Connexion)", type="password", key="login_password")

        if st.button("Connexion", key="login_button"):
            if login_email and login_password:
                result = signin(login_email, login_password)
                if result['success']:
                    st.session_state.logged_in = True
                    st.session_state.user_info = result['user'] # Stocke les infos de l'utilisateur Firebase
                    st.session_state.page = "dashboard"
                    st.success(f"Connexion réussie pour {login_email}!")
                    st.rerun()
                else:
                    st.error(f"Erreur de connexion : {result['error']}")
            else:
                st.warning("Veuillez entrer votre email et mot de passe.")

    with col2: # Section Inscription
        st.subheader("Créer un compte")
        signup_email = st.text_input("Email (Inscription)", key="signup_email")
        signup_password = st.text_input("Mot de passe (Inscription)", type="password", key="signup_password")
        ...
        if st.button("S'inscrire", key="signup_button"):
            if signup_email and signup_password and signup_password_confirm:
                if signup_password == signup_password_confirm:
                    ...
                    if result['success']:
                        st.success(f"Compte créé pour {signup_email} ! Vous pouvez maintenant vous connecter.")
                        # On ne connecte pas automatiquement, l'utilisateur doit se connecter après inscription
                    else:
                        st.error(f"Erreur d'inscription : {result['error']}")
                else:
                    st.error("Les mots de passe ne correspondent pas.")
            else:
                st.warning("Veuillez remplir tous les champs d'inscription.")

# --- Logout Function (MODIFIED) ---
def app_logout(): # Renommée pour éviter conflit avec le mot-clé 'logout' si importé directement
    st.session_state.logged_in = False
    st.session_state.user_info = None
    st.session_state.page = "login"
    st.session_state.chat_history = []
    # Firebase gère la session côté serveur, nous n'avons pas besoin d'appeler auth.sign_out() ici
    # car pyrebase gère les tokens automatiquement. Le simple fait de supprimer les infos
    # de la session Streamlit suffit pour "déconnecter" l'utilisateur de l'interface.
    st.rerun()
...
if not st.session_state.logged_in:
    ...
    st.sidebar.button("Se déconnecter", on_click=app_logout) # Utilisez la nouvelle fonction de déconnexion

    # Afficher l'email de l'utilisateur connecté
    if st.session_state.user_info and 'email' in st.session_state.user_info:
        st.sidebar.write(f"Connecté en tant que : {st.session_state.user_info['email']}")
    else:
        st.sidebar.write("Connecté (email non disponible)") # Fallback
    ...
    # Dans un système réel, vous pourriez stocker le rôle de l'utilisateur dans Firebase
    # (par exemple, dans Firestore ou Realtime Database) et le récupérer ici.
    # Pour ce PFE, nous allons simplifier et donner accès à toutes les fonctionnalités
    # une fois que l'utilisateur est connecté via Firebase.
    # Si vous voulez encore différencier les rôles BI/AI, il faudrait ajouter
    # un mécanisme pour assigner et récupérer ces rôles depuis Firebase.
